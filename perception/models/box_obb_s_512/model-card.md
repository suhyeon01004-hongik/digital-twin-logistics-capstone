...
학습 인자는 `training_args.yaml`에 보존했습니다. 모델 바이너리는 Git에
커밋하지 않고 필요하면 GitHub Release, Drive 또는 모델 저장소로 배포합니다.
...
clone한 환경에서는 팀 내부 보관본 또는 Release 자산에서 `Runtime file` 위치로
복사한 뒤 `python3 scripts/validate_runtime_assets.py`를 실행합니다.
